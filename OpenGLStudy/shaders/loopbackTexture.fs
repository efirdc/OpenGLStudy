...
void main()
{
  ...
  float specular = pow(max(dot(freqSurfaceNorm3D, halfwayDir), 0.0), 8) * specularStrength;
  
  vec4 diffuseColor = texture(lightColorCurve, (diffuse + 1.0) * 0.5);
  vec4 ambientColor = texture(frequencyColorCurve, freqMid);
