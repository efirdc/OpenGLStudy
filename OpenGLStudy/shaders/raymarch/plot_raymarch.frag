...
uniform float densityMean;
uniform float densityDistribution;
...
float implicitFunc(vec3 p)
{
	float z_n2 = 1 / (p.z * p.z);

	float val = p.x * exp(z_n2 - 1.0) + p.y * sqrt(exp(1.0 - z_n2));

	float a = (val - densityMean) / densityDistribution;
	return exp( -0.5 * a * a);
}
...
vec3 getShadowTransmittance(vec3 rayPos, vec3 boxMin, vec3 boxMax)
{
	...
	for (int j = 0; j < numShadowSteps; j++)
	{
		...
		float density = implicitFunc(lightRayPos);
		shadow *= exp(density * scatteringData[0].extinction * shadowStepSize);
		...
		in vec3 boxMax)
{
	...
	float density = implicitFunc(rayPos);
	if (density > 0.0000000001)
	{
		vec3 shadow = getShadowTransmittance(rayPos, boxMin, boxMax).xyz;
...
void main()
{
	...
	float extents = 128;
	vec3 boxMax = boxMaximum;
	vec3 boxMin = boxMinimum;
